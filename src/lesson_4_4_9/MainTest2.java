...
//            Sendable parsed = mail;
//            for (int i = 0; i < otherPersons.length; i++) {
//                parsed = otherPersons[i].processMail(parsed);
//            }
...
//        }
        ...
        public Sendable processMail(Sendable mail) {
            Sendable parsed = mail;
            for (MailService service : otherPersons) {
                parsed = service.processMail(parsed);
            }
            return realMailService.processMail(parsed);
        }
    }
    ...
    public static class Inspector implements MailService {
        ...
        public Sendable processMail(Sendable mail) {
            if(mail.getClass() == MailPackage.class) {
                Package pac = ((MailPackage)mail).getContent();
                String content = pac.getContent().toLowerCase();
                if(content.indexOf("stones instead of ") == 0) {
                    throw new StolenPackageException();
                } else if(content.contains(WEAPONS.toLowerCase()) || content.contains(BANNED_SUBSTANCE.toLowerCase())){
